...
precio_venta = st.number_input("Precio de venta propuesto (€)", value=290000)
comision_venta_pct = st.number_input("Comisión de venta (%)", value=3.0)
...
variacion_coste_reforma = st.slider("Variación en coste de obra (%)", -10, 10, 0)

# CÁLCULOS
precio_venta_final = precio_venta * (1 + variacion_precio_venta / 100)
coste_obra = superficie_reformar * coste_m2_reforma * (1 + variacion_coste_reforma / 100)
...
inversion_total = precio_compra + comision_compra + gastos_legales + gastos_admin + impuesto_compra + ibi + tasacion + registro + reforma_total
...
st.markdown(f"**Precio de venta final:** €{precio_venta_final:,.2f}")
st.markdown(f"**Coste total de reforma (con IVA):** €{reforma_total:,.2f}")
...
if st.button("📄 Descargar resumen en PDF"):
    ...
    pdf.add_page()
    try:
        pdf.image("ADCO_LOGO_SIMPLE.png", x=10, y=8, w=33)
    except:
        pass
    ...
    pdf.ln(10)
    pdf.multi_cell(0, 10, f"Ubicación: {ubicacion}")
    pdf.cell(200, 10, txt=f"Superficie: {superficie} m²", ln=True)
    pdf.cell(200, 10, txt=f"Precio de compra: €{precio_compra:,.2f}", ln=True)
    pdf.cell(200, 10, txt=f"Precio de venta final: €{precio_venta_final:,.2f}", ln=True)
    ...
    pdf.cell(200, 10, txt=f"Rentabilidad anualizada (TIR): {tir * 100:.2f}%", ln=True)
    pdf.ln(15)
